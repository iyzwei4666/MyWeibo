...
class PhotoArticlePresenter implements IPhotoArticle.Presenter {
    ...
    private List<ImageResult.DataBean> dataList = new ArrayList<>();
...
//                        doSetAdapter(list);
        ...
        RetrofitFactory.getRetrofit().create(IPhotoApi.class).getPhotoArticle("明星" , "全部" ,"女明星##内地")
                .subscribeOn(Schedulers.io())
                .switchMap((Function<ImageResult , Observable<ImageResult.DataBean>>) imageResult -> {
                   List data =  imageResult.getData() ;
                    if (data.size() > 0)
                        data.remove(data.size() - 1);
                    return Observable.fromIterable(data);
...
//                    ErrorAction.print(throwable);
    ...
    @Override
    public void doSetAdapter(List<ImageResult.DataBean> dataBeen) {
